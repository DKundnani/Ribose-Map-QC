...
option_list = list(
  make_option(c("-c", "--counts"), type="character", default=NULL, 
              help="counts.tsv generated with lib, counts in chrM, counts in ncul and total counts", metavar="filetype"),
  make_option(c("-l", "--libinfo"), type="character", default=NULL, 
              help="file with lib,Celltype,Fragmentation", metavar="filetype")
...
writeLines(paste("Input file name:", opt$counts))
...
QC<-read.table(opt$counts,sep="\t",header=T)
libinfo<-read.table(opt$libinfo,sep="\t",header=F); colnames(libinfo)<-c("Lib","Group","RE")
QC$Library<-paste(libinfo$Lib,libinfo$RE, sep='-')
...
png("Raw Nuclear counts horizontal.png", width=12, height=7,units= "in",  res=600)
ggplot(QC , aes(x=Library, y=Counts_Nuclear/1e6))+
  geom_bar(stat="identity", fill="steelblue")+
  coord_flip()+
...
png("Raw Nuclear counts grouped horizontal.png", width=18, height=7,units= "in",  res=600, bg="transparent")
...
png("Raw Mitochondria counts horizontal.png", width=12, height=7,units= "in",  res=600)
ggplot(QC , aes(x=Library, y=Counts_Mito/1e6))+
  geom_bar(stat="identity", fill="steelblue")+
  coord_flip()+
  scale_x_discrete(limits=rev(QC$Library),labels= rev(QC$Library))+
...
png("Nuclear and Mitochondria Percentage legend.png",width=2, height=2,units= "in",  res=600, bg="transparent")
...
png("Nuclear and Mitochondria Percentage horizontal.png", width=12, height=8,units= "in",  res=1000, )
...
png("Nuclear percent grouped horizontal.png", width=12, height=7,units= "in",  res=600)
...
png("Mitochondrial percent grouped horizontal.png", width=12, height=7,units= "in",  res=600)
ggplot(groupedcounts , aes(x=Group, y=Mitoperc*100))+
  geom_boxplot(fill="#93A1E5")+
  geom_point()+
  coord_flip()+
  scale_x_discrete(limits=rev(unique(groupedcounts$Group)),labels= rev(unique(groupedcounts$Group)))+
  scale_y_continuous(expand=c(0,0), n.breaks=10, limits = c(0,120))+
  #scale_y_break(c(80, 90), scales=2)+
  ggtitle("Ribonucleotide percentage in Mitochondria")+
  xlab("")+
  ylab("Percentage")+
  theme_classic()+
  theme(plot.title = element_text(size=30, face="bold",hjust = 0.5), 
